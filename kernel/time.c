...
static int k_schedule_alarms ()
{
	...
	while ( first != NULL )
	{
		if ( time_cmp ( &first->alarm.exp_time, &ref_time ) <= 0 )
		{
			...
			resched_thr += kthreadq_release_all ( &first->queue );
...
static void k_alarm_add ( kalarm_t *kalarm )
{
	...
	/* block thread? */
	if ( kalarm->thread && kalarm->active &&
		( kalarm->alarm.flags & IPC_WAIT ) )
	{
		kthread_enqueue ( kalarm->thread, &kalarm->queue );
		reschedule = 1;
	}
 ...
 */
int k_alarm_new ( void **id, alarm_t *alarm, int priv )
{
	kalarm_t *kalarm;
	...
	*id = kalarm; /* return value = handler */
	...
	if ( priv == SYSCALL )
		kalarm->thread = kthread_get_active ();
	else /* priv == KERNELCALL */
		kalarm->thread = NULL;
...
int k_alarm_remove ( void *id )
{
	...
	reschedule = kthreadq_release_all ( &kalarm->queue );
...
int sys__alarm_new ( void *p )
{
	...
	id = U2K_GET_ADR ( id, kthread_get_process (NULL) );
	alarm = U2K_GET_ADR ( alarm, kthread_get_process (NULL) );

	ASSERT_ERRNO_AND_EXIT ( id && alarm, E_INVALID_HANDLE );

	return k_alarm_new ( id, alarm, SYSCALL );
}
...
int sys__wait_for_alarm ( void *p )
{
	...
	if ( kalarm->active )
	{
		if ( wait & IPC_WAIT )
		{
			kthread_enqueue ( NULL, &kalarm->queue );
			kthreads_schedule ();
		}
		else {
