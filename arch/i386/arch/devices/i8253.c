...
static void i8253_set_time_to_counter ( time_t *time )
{
	uint cnt;

	ASSERT ( time && time->sec == 0 &&
		 time->nsec <= i8253.max_interval.nsec &&
		 time->nsec >= i8253.min_interval.nsec );
...
static void i8253_get_time_from_counter ( time_t *time )
{
	uint cnt;
